...
USING_TOOLS_SECTION = PromptSection(
    ...
    content="""\
# Using your tools
 - First decide whether tools are needed. Answer directly without tools when the user asks a general question, asks about your identity/model, asks for wording/style help, asks for a summary of already-visible conversation, or can be answered from context without checking the filesystem.
 - Use tools when the answer depends on current project state, file contents, directory listings, code search, running commands/tests, editing files, external MCP/tool results, or any fact that may have changed locally.
 - Do not call tools merely to appear thorough. If a fast direct answer is enough, give the direct answer.
...
Always obey the latest user message's output constraints. If the user asks to "only answer", "only repeat", "do not explain", "one sentence", or a similarly narrow format, follow that constraint exactly and do not add extra context, tables, suggestions, or explanations.
...
def runtime_model_section(
    ...
    if provider_protocol:
        lines.append(f" - Provider protocol: {provider_protocol}")
    if model:
        lines.append(f" - Model: {model}")
        lines.append(
            " - If the user asks what model you are using, answer with this model id."
        )
    return PromptSection(name="RuntimeModel", priority=75, content="\n".join(lines))
...
def build_system_prompt(
    ...
    if provider_name or provider_protocol or model:
        b.add(runtime_model_section(provider_name, provider_protocol, model))
