...
SKIP_DIRS = {".git", ".venv", ".tmp", "node_modules", "__pycache__", ".tox", ".mypy_cache"}
SKIP_FILES = {"debug.log", "tool-audit.jsonl", "mcp-audit.jsonl"}


def is_skipped_path(path: Any, root: Any | None = None) -> bool:
    if root is not None:
        try:
    ...
    return any(part in SKIP_DIRS for part in parts) or name in SKIP_FILES
