...
def build_time_gap_message(
    ...
    threshold_hours: int = TIME_GAP_REMINDER_HOURS,
) -> Message | None:
    """Return a resume reminder when a session has been inactive for a while."""
    ...
    return Message(
        ...
        content=(
            "<system-reminder>\n"
            f"距离上次会话已经过去约 {hours} 小时，代码可能有变更。"
            "继续回答前请根据需要重新检查相关文件和项目状态。\n"
            "</system-reminder>"
        ),
    )
