...
class MewCodeApp(App):
    ...
    def _start_agent_task(self, text: str, is_notification: bool = False) -> None:
        task = asyncio.create_task(
            self._send_message(text, is_notification=is_notification)
        )
        task.add_done_callback(self._log_agent_task_done)
        self._agent_task = task

    def _log_agent_task_done(self, task: asyncio.Task[None]) -> None:
        if task.cancelled():
            log.info("agent task cancelled")
            return
        exc = task.exception()
        if exc is not None:
            log.error("agent task failed", exc_info=(type(exc), exc, exc.__traceback__))
    ...
    async def _send_message(self, text: str, is_notification: bool = False) -> None:
        try:
            await self._send_message_impl(text, is_notification=is_notification)
        except asyncio.CancelledError:
            raise
        except Exception as e:
            log.exception("message handling failed")
            self._show_error(f"Message handling failed: {e}")
            self._finish_streaming()
            try:
                self.query_one("#chat-input", ChatInput).focus()
            except Exception:
    ...
    async def _send_message_impl(self, text: str, is_notification: bool = False) -> None:
    ...
    async def _process_task_notifications(self) -> None:
        ...
        self._start_agent_task("", is_notification=True)
    ...
    async def _init_mcp(self) -> None:
        ...
        try:
            errors = await asyncio.wait_for(
                manager.register_all_tools(self.registry),
                timeout=MCP_INIT_TIMEOUT_SECONDS,
            )
            self._mcp_errors = errors
        except TimeoutError:
            errors = [
                f"MCP initialization timed out after {MCP_INIT_TIMEOUT_SECONDS:.0f}s"
            ]
            self._mcp_errors = errors
            await manager.shutdown()
        except Exception as e:
            errors = [f"MCP initialization failed: {e}"]
            self._mcp_errors = errors
            await manager.shutdown()
        finally:
