...
def main() -> None:
    ...
    if os.environ.get("MEWCODE_NO_ALT_SCREEN") == "1":
        from mewcode.driver import NoAltScreenDriver

        app_kwargs["driver_class"] = NoAltScreenDriver

    app = MewCodeApp(**app_kwargs)
    app.run()
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    ...
    try:
        if config.mcp_servers:
            ...
            errors = await mcp_manager.register_all_tools(registry)
            for err in errors:
                print(f"MCP warning: {err}", file=sys.stderr, flush=True)
            tool_sections: list[str] = []
            for server in config.mcp_servers:
                tool_names = [
                    t.name for t in registry.list_tools()
                    if t.name.startswith(f"mcp__{server.name}__")
                ]
                if tool_names:
            ...
            if tool_sections:
                conv_mcp_note = (
                    ...
                    "The following MCP servers are connected. Use their tools when useful.\n\n"
                    + "\n\n".join(tool_sections)
                )
            else:
                conv_mcp_note = ""
        ...
        if conv_mcp_note:
            conv.add_system_reminder(conv_mcp_note)
