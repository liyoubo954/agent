...
@pytest.mark.asyncio
async def test_mcp_command_reports_connecting() -> None:
    from mewcode.commands.handlers.mcp import handle_mcp

    ui = MockUI()
    ui._mcp_connecting = True

    await handle_mcp(_make_context(ui=ui))

    assert ui.messages == ["MCP servers are still connecting"]


@pytest.mark.asyncio
async def test_mcp_command_reports_connection_errors() -> None:
    from mewcode.commands.handlers.mcp import handle_mcp

    ui = MockUI()
    ui._mcp_errors = ["MCP server 'github': HTTP 400 Bad Request"]

    await handle_mcp(_make_context(ui=ui))

    assert "No MCP servers connected" in ui.messages[0]
    assert "HTTP 400 Bad Request" in ui.messages[0]
