...
def test_agent_accepts_runtime_model_metadata():
    agent = Agent(
        MockLLMClient([]),
    ...
    assert agent.provider_name == "deepseek-official"
    assert agent.model == "deepseek-v4-pro"

def test_system_prompt_includes_runtime_model_metadata():
    sp = build_system_prompt(
        provider_name="deepseek-official",
        provider_protocol="openai-compat",
        model="deepseek-v4-pro",
    )

    assert "Provider: deepseek-official" in sp
    assert "Provider protocol: openai-compat" in sp
    assert "Model: deepseek-v4-pro" in sp
